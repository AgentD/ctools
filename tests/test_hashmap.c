...
int main( void )
{
    ...
    for( i=0; i<sizeof(test_vals)/sizeof(test_vals[0]); ++i )
    {
        for( j=0; j<i; ++j )
        {
            if( tl_hashmap_get(&map,test_keys+j) )
                return EXIT_FAILURE;
        }
        for( ; j<sizeof(test_vals)/sizeof(test_vals[0]); ++j )
        {
            if( *((long*)tl_hashmap_get(&map,test_keys+j)) != test_vals[j] )
                return EXIT_FAILURE;
        }
    ...
    for( i=0; i<sizeof(test_vals)/sizeof(test_vals[0]); ++i )
    {
        if( i )
        {
            if( !tl_hashmap_get( &map, test_keys ) )
                return EXIT_FAILURE;
            if( *((long*)tl_hashmap_get(&map,test_keys)) != test_vals[i-1] )
                return EXIT_FAILURE;
        }
        else if( tl_hashmap_get( &map, test_keys ) )
        {
            return EXIT_FAILURE;
        }

        tl_hashmap_add( &map, test_keys, test_vals+i );

        if( !tl_hashmap_get( &map, test_keys ) )
            return EXIT_FAILURE;

        if( *((long*)tl_hashmap_get(&map,test_keys)) != test_vals[i] )
            return EXIT_FAILURE;
    }
    ...
    for( i=0; i<sizeof(test_vals)/sizeof(test_vals[0]); ++i )
    {
        ...
        if( *((long*)tl_hashmap_get(&map,test_keys)) != test_vals[j] )
            return EXIT_FAILURE;
