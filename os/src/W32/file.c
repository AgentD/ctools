				       ...
				       size_t count, int flags)
{
	...
	if (flags & TL_MAP_WRITE) {
		...
		prot = (flags & TL_MAP_EXECUTE) ? PAGE_EXECUTE_READ :
						  PAGE_READONLY;
	}

	if (flags & TL_MAP_WRITE)
		viewprot |= FILE_MAP_WRITE;
	if (flags & TL_MAP_READ)
		viewprot |= FILE_MAP_READ;
	if (flags & TL_MAP_EXECUTE)
		viewprot |= FILE_MAP_EXECUTE;
	if (flags & TL_MAP_COW)
		viewprot |= FILE_MAP_COPY;
