...
typedef enum {
	/**
	 * \brief If the splice operation is not supported,
	 *        don't use the fallback
	 *
	 * By default, if the underlying OS implementation does no support
	 * splice for the supplied streams, tl_iostream_splice will use a
	 * fallback implementation that reads and writes data in a loop.
	 * However, this fallback is no longer atomic. If a read succeeds
	 * but a subsequent write fails, the fallback cannot put the data
	 * back into the stream and the data is lost. So in some cases,
	 * you might prefere tl_iostream_splice to fail immediately and
	 * handle the case in a different way.
	 */
