    ...
    while( this->charcount )
    {
        ...
        for( i=0; (*ptr & 0xC0)==0x80; i+=6 )
            cp |= (*(ptr--) & 0x3F) << i;
             ...
             if( (*ptr & 0xF8) == 0xF0 ) { cp |= (*ptr & 0x07) << i; }
        else if( (*ptr & 0xF0) == 0xE0 ) { cp |= (*ptr & 0x0F) << i; }
        else if( (*ptr & 0xE0) == 0xC0 ) { cp |= (*ptr & 0x1F) << i; }
        else                             { cp |= *ptr;               }
    ...
    while( 1 )
    {
        ...
        if( !cp )
            break;
             ...
             if( (cp & 0xE0) == 0xC0 ) { cp &= 0x1F; }
        else if( (cp & 0xF0) == 0xE0 ) { cp &= 0x0F; }
        else if( (cp & 0xF8) == 0xF0 ) { cp &= 0x07; }
