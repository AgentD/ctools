...
int tl_process_args(tl_option * options, size_t num_options,
		    int *argc, char **argv, tl_option_error_handler handler)
{
	int i = 1, status;
	const char *arg;
	size_t j = 0, len;
	...
	while (i < (*argc)) {
		...
		if (!strcmp(argv[i], "--")) {
			...
			arg = argv[i] + 2;
			if (find_string
			    (options, num_options, arg, TL_LONG_FLAG, &j)) {
				*(options[j].field) |= options[j].value;
				goto removeone;
			}
			if (find_string
			    (options, num_options, arg, TL_LONG_OPTION, &j)) {
				if (i == (*argc - 1) || argv[i + 1][0] == '-'
				    || argv[i + 1][0] == '+')
					goto missingarg;
				options[j].handle_option(options + j,
							 argv[i + 1]);
				goto removetwo;
			}
			...
			if (can_resolve_chars
			    (options, num_options, arg, TL_SHORT_FLAG)) {
				while (*arg) {
					find_flag(options, num_options,
						  *(arg++), TL_SHORT_FLAG, &j);
					*(options[j].field) |= options[j].value;
				}
				goto removeone;
			...
			if (find_string
			    (options, num_options, arg, TL_SHORT_OPTION, &j)) {
				if (i == (*argc - 1) || argv[i + 1][0] == '-'
				    || argv[i + 1][0] == '+')
					goto missingarg;
				options[j].handle_option(options + j,
		...
		status =
		    handler ? handler(NULL, argv[i],
				      TL_OPT_UNKNOWN) : TL_OPT_FAIL;
 handleaction:
		if (status == TL_OPT_IGNORE)
			goto skip;
		if (status == TL_OPT_REMOVE)
			goto removeone;
		return 0;
 missingarg:
		status =
		    handler ? handler(options + j, 0,
				      TL_OPT_MISSING_ARGUMENT) : TL_OPT_FAIL;
		goto handleaction;
 removetwo:	remove_option(argc, argv, i, 2);
		continue;
 removeone:	remove_option(argc, argv, i, 1);
		continue;
 skip:		++i;
	}
